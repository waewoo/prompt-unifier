...
### Situation
The user is starting a new Ansible project and needs an `ansible.cfg` file to define the project's configuration, such as inventory location, roles path, and default behaviors.

### Challenge
Generate a single, complete `ansible.cfg` file. The file must be well-commented and set sensible defaults that align with the project's established standards.
...
- **Privilege Escalation**: Configure default privilege escalation settings (`become`, `become_method`, `become_user`).
- **SSH Settings**: Include common SSH optimizations in `ssh_connection` section to speed up connections.
...
"I need a standard `ansible.cfg` file for my new project. The inventory will be in the `./inventory` directory and roles in `./roles`."